...
exports.register = function(server, options, next) {
  ...
  server.method('getFacebookPhotoURL', function(token, id, next) {
    var requestURL = 'https://graph.facebook.com/v2.4/'+id+'/picture';
    /*
     * Picture fb graph edge responds with a redirect to image
     * To get the JSON response we have to pass redirect=0
     **/
    request.get(requestURL)
            .query({height: 320, redirect: 0, access_token: token})
            .end(function(err, res) {
              if (err) {
                next(boom.create(404, 'Unable to get profile photo', err))
              } else {
                var fbJson = JSON.parse(res.text);
                next(null, fbJson.data.url)
              }
            })
  })
